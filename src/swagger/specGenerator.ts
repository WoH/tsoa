...
export abstract class SpecGenerator {
  ...
  private determineTypesUsedInEnum(anEnum: Array<string | number | boolean>) {
  ...
  protected decideEnumType(anEnum: Array<string | number | boolean>): 'string' | 'number' | 'boolean' {
    ...
    if (typesUsedInEnum.has('string') && typesUsedInEnum.size === 1) {
      ...
      return 'number';
    } else if (typesUsedInEnum.has('boolean') && typesUsedInEnum.size === 1) {
      return 'boolean';
    } else {
      // tslint:disable-next-line: no-console
      console.warn(`Swagger/OpenAPI does not support enums with both strings and numbers but found that for enum ${JSON.stringify(anEnum)}`);
      return 'string';
    }
  }

  protected getSwaggerTypeForEnumType(enumType: Tsoa.EnumType): Swagger.Schema {
    const type = this.decideEnumType(enumType.enums);
    return { type, enum: enumType.enums };
  }
}
